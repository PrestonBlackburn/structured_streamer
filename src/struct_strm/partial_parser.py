...
class FormState(ABC):
    ...
    async def next_state(
        self
    ) -> AsyncGenerator[Union[Type, 'FormState'], None]:
        ...
...
class FormDescState(FormState):
    ...
    async def execute(self, buffer:str, current_chunk:str) -> AsyncGenerator[str, None]:
        _logger.debug("execute FormDescState")
        item_termintors = {'"', "'", ","} # TODO - see if we can prompt for better terminators
...
class FormNameState(FormState):
    ...
    async def execute(self, buffer:str, current_chunk:str) -> AsyncGenerator[str, None]:
        ...
        item_termintors = {'"', "'", ",", f'{self.field_description_key}'} # TODO - see if we can prompt for better terminators
        pair_terminators = {'}', ']', '},', '},{"', '}]', f'"{self.field_description_key}"'}
        ...
        if not self.in_state and f'"{self.field_name_key}":' in buffer:
            ...
            # init new row
            self.field_idx += 1
            self.results.update({self.field_idx: {self.field_name_key: "", self.field_description_key: ""}})
...
async def parse_form_json_fsm(response_stream: AsyncGenerator[str, None]) -> AsyncGenerator[dict, None]:
    results = {}
    buffer = ""
    state = FormInitState(results)
    async for chunk in response_stream:
        buffer = buffer + chunk
