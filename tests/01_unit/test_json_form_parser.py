import pytest
from struct_strm import parse_form_json_ts, parse_form_json_fsm
...
async def test_parse_form_json():
    ...
    # ]
    last_items = {
        0: {"field_name": "fruits", "field_placeholder": "apple orange strawberry"},
        1: {"field_name": "appliance", "field_placeholder": "blender mixer toaster"},
        2: {"field_name": "dishes", "field_placeholder": "plate bowl"},
    }
    print(all_items)
...
@pytest.mark.asyncio
async def test_parse_form_ts():

    stream = simulate_stream_form_struct()

    all_items = []
    async for items in parse_form_json_ts(stream):
        print(items)
        all_items.append(items)
...
@pytest.mark.asyncio
async def test_openai_parse_form_ts():
